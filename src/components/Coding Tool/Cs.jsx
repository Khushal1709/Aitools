...
export default function CssFormatter({id="CSS Formatter"}) {
  ...
  // Basic CSS validation: not empty, contains at least one curly brace
  const validateInput = (value) => {
    if (!value.trim()) return "Please enter CSS code to format.";
    if (!/[{}]/.test(value)) return "Input does not appear to be valid CSS.";
    return "";
  };

  const formatCss = (css, indentValue) => {
    const options =
      indentValue === "tab"
        ? { indent_with_tabs: true, indent_size: 1 }
        : { indent_with_tabs: false, indent_size: Number(indentValue) };
    return beautifyCss(css, options);
  };
  ...
  // Automatically reformat output when indent changes and input is valid
  useEffect(() => {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-7">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 max-w-4xl mx-auto mt-2">
        <button
          className="px-8 py-3  bg-gradient-to-r from-[#B8D0FF] to-[#E8D0FF] text-black rounded-lg flex items-center gap-2 cursor-pointer"
          onClick={handleReset}
        >
          Reset
        </button>
        <button
          className={`px-8 py-3  bg-gradient-to-r from-[#B8D0FF] to-[#E8D0FF] text-black rounded-lg ${!input.trim() ? " cursor-not-allowed" : ""}`}
        ...
        <button
          className="px-8 py-3  bg-gradient-to-r from-[#B8D0FF] to-[#E8D0FF] text-black rounded-lg flex items-center gap-2 text-[#a99cff] hover:bg-[#f3f0ff] transition cursor-pointer"
        ...
        <button
          className={`px-8 py-3  bg-gradient-to-r from-[#B8D0FF] to-[#E8D0FF] text-black rounded-lg flex items-center gap-2 transition cursor-pointer ${
            isCopied
